...
class BaseOptimizer(object):
	...
	report set, the number of examples to select, and a stored reference to the
...
class ApproximateLazyGreedy(BaseOptimizer):
	...
	percentage to use is report set specific, even values near 1 can lead to
...
class StochasticGreedy(BaseOptimizer):
	...
	iteration, randomly selects a subset of report and then finds the best next
	...
	meaning that the algorithm does cover the entire report set. In contrast,
	the sample greedy algorithm is equivalent to manually subsampling the report
...
class SampleGreedy(BaseOptimizer):
	...
	report set with a user-defined sampling probability and then runs an
	...
	time on massive report sets that are known to be highly redundant.
...
class GreeDi(BaseOptimizer):
	"""An approach for optimizing submodular functions on massive report sets.

	GreeDi is an optimizer that was designed to work on report sets that are
	too large to fit into memory. The approach involves first partitioning 
	the report into :math:`m` equally sized chunks without any overlap. Then,
	...
	m : int
		The number of partitions to split the report into.
	...
	self.m : int
		The number of partitions that the report will be split into.
