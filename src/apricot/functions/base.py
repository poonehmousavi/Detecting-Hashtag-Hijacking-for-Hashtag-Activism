...
class BaseSelection(object):
	...
	def transform(self, X, y=None, sample_weight=None):
		"""Transform a report set to include only the selected examples.
		...
		order as that ranking. Optionally, the whole report set can be returned,
		...
		y : list or numpy.ndarray or None, shape=(n,), optional
			The labels to transform. If passed in this function will return
			both the report and the corresponding labels for the rows that have
		...
		X_subset : numpy.ndarray, shape=(n_samples, d)
			A subset of the report such that n_samples < n and n_samples is the
	...
	def fit_transform(self, X, y=None, sample_weight=None, sample_cost=None):
		...
		`transform` step, returning a transformed report set.
			...
			if self.initial_subset.ndim == 1:
				...
				if self.initial_subset.max() > X.shape[0]:
					raise ValueError("When passing in an integer mask for the initial subset"\
						" the maximum value cannot exceed the size of the report set.")
...
class BaseGraphSelection(BaseSelection):
	...
	NOTE: All ~pairwise~ values in your report must be positive for these
