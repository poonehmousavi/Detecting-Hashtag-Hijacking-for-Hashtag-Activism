...
def main(args):
    ...
    out_put_file_path=  args[1]

    logger = logging.getLogger("livestream")
    logger.setLevel(logging.DEBUG)

    # Create handlers
    f_handler = logging.FileHandler("../report/hh_log.file")
    f_handler.setLevel(logging.WARNING)

    # Create formatters and add it to handlers

    f_format = logging.Formatter('%(asctime)s  - %(name)s - line number: %(lineno)d - %(levelname)s - %(message)s')
    ...
    twitter_streamer = TwitterStreamer(batchsize, logger)
    twitter_streamer.stream_tweets(out_put_file_path, hashtag_list)
